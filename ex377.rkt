...
; XEnum.v2 -> XEnum.v2
; replaces all "hello"s in the given enumeration with "bye"
(define (replace-hello enum)
  (cons 'ul (map replace-hello-item (xexpr-content enum))))

(check-expect (replace-hello en0) en0)
(check-expect (replace-hello en1) `(ul (li (word ((text "bye"))))
                                       (li ,en0) (li ,w4) (li ,w5)))

; XItem.v2 -> XItem.v2
; replaces all "hello"s in the given item with "bye"
    ...
    (cond
      [(word? content)
       (if (string=? "hello" (word-text content))
           '(li (word ((text "bye"))))
           item)]
      [else `(li ,(replace-hello content))])))

(check-expect (replace-hello-item `(li ,w2)) `(li ,w2))
(check-expect (replace-hello-item `(li ,w1)) `(li (word ((text "bye")))))
(check-expect (replace-hello-item `(li (ul (li ,w1) (li ,en0) (li ,w1))))
              `(li (ul (li (word ((text "bye"))))
                       (li ,en0)
                       (li (word ((text "bye")))))))
