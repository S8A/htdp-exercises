...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex336) (read-case-sensitive #t) (teachpacks ((lib "abstraction.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "abstraction.rkt" "teachpack" "2htdp")) #f)))
; .: Data definitions :.
...
(define (how-many dir)
  (local (; Dir.v3 -> N
          (define (how-many-in-dir d)
            (+ (length (dir-files d))
               (how-many-in-lod (dir-dirs d))))
          ; Dir* -> N
          (define (how-many-in-lod lod)
            (cond
              [(empty? lod) 0]
              [(cons? lod)
               (+ (how-many-in-dir (first lod))
                  (how-many-in-lod (rest lod)))])))
    (how-many-in-dir dir)))
