...
(define (static-distance le0)
  (local (; Lam [List-of Symbol] -> "Lam"
          ...
          (define (stdist/a le declareds)
            (cond
              [(is-var? le)
               (- (length declareds) (length (memv le declareds)))]
              [(is-λ? le)
               (local ((define para (λ-para le))
                       (define body (λ-body le))
                       (define newd (cons para declareds)))
                 (list 'λ (stdist/a body newd)))]
              [(is-app? le)
               (list (stdist/a (app-fun le) declareds)
                     (stdist/a (app-arg le) declareds))])))
    (stdist/a le0 '())))
