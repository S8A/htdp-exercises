...
(define (find-name n los)
  (ormap (lambda (s) (string-contains-ci? n s)) los))
...
(define (all-start-with letter los)
  (andmap (lambda (s) (string-ci=? letter (string-ith s 0))) los))
...
(define (all-under-width width los)
  (andmap (lambda (s) (<= (string-length s) width)) los))
...
(define (none-over-width width los)
  (not (ormap (lambda (s) (> (string-length s) width)) los)))
