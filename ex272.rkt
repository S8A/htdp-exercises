...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname ex272) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp")) #f)))
; [X] [List-of X] [List-of X] -> [List-of X]
; Appends list2 to list1 using foldr
(define (append-from-foldr list1 list2)
  (foldr cons list2 list1))

(check-expect (append-from-foldr (list 1 2 3) (list 4 5 6 7 8))
              (list 1 2 3 4 5 6 7 8))

; Q: What happens if you replace foldr with foldl?
; A: It will append list2 to the reverse of list1
(define (append-from-foldl list1 list2)
...
(check-expect (sum-list '(4 5 6 7 8)) 30)
...
(check-expect (product-list '(4 5 6 7 8)) 6720)
...
(define (stack-horiz l)
  (foldr beside empty-image l))

(check-expect (stack-horiz lim1)
              (beside im1 (beside im2 (beside im3 empty-image))))
(check-expect (stack-horiz lim2)
...
(define (stack-horiz-alt l)
  (foldl beside empty-image (reverse l)))

(check-expect (stack-horiz-alt lim1)
              (beside im1 (beside im2 (beside im3 empty-image))))
(check-expect (stack-horiz-alt lim2)
              (beside im4 (beside im5 (beside im6 empty-image))))
...
(check-expect (stack-vert lim1)
              (above im1 (above im2 (above im3 empty-image))))
(check-expect (stack-vert lim2)
