...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname ex204) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp")) #f)))
...
(define (select-album-date alb d lt)
  (cond
    [(empty? lt) '()]
    [(cons? lt)
     (if (and (belongs-to-album alb (first lt))
              (played-after d (first lt)))
         (cons (first lt) (select-album-date alb d (rest lt)))
         (select-album-date alb d (rest lt)))]))
...
(define (select-albums lt)
  (create-list-by-album (select-album-titles/unique lt) lt))
...
; creates a list of lists containing tracks from lt that belong to each album
; in the given list of albums
(define (create-list-by-album loa lt)
  (cond
    [(empty? loa) '()]
    [(cons? loa)
     (cons (select-album (first loa) lt)
           (create-list-by-album (rest loa) lt))]))

(check-expect (create-list-by-album (list "Leviathan") lt0) (list '()))
(check-expect (create-list-by-album (list "Antichrist Superstar") lt2)
              (list (list tbp trg)))
(check-expect (create-list-by-album (list "Antichrist Superstar"
                                          "Leviathan") lt3)
              (list (list tbp trg) (list bat)))
