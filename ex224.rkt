...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname ex224) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp")) #f)))
; Constants :.
...
(define (missiles-render lom im)
  (cond
    [(empty? lom) im]
    [(cons? lom)
     (missile-render (first lom) (missiles-render (rest lom) im))]))
...
(define (move-missiles lom)
  (cond
    [(empty? lom) '()]
    [(cons? lom)
     (if (missile-out-of-frame? (first lom))
...
; Missile -> Boolean
; Checks if the missile is beyond the top edge of the frame.
(define (missile-out-of-frame? m)
  (<= (posn-y m) (- MISSILE-HALF-SIZE)))

(check-expect (missile-out-of-frame? missile0) #false)
(check-expect (missile-out-of-frame? missile1) #false)
(check-expect (missile-out-of-frame? missile2) #false)
(check-expect (missile-out-of-frame? missile3) #false)
(check-expect (missile-out-of-frame? missile4) #false)
(check-expect (missile-out-of-frame? (make-posn X-CENTER (- MISSILE-HALF-SIZE)))
              #true)
(check-expect (missile-out-of-frame? (make-posn 10 -20)) #true)
...
(define (ufo-hit? u lom)
  (cond
    [(empty? lom) #false]
    [(cons? lom)
     (or (missile-hit-ufo? (first lom) u)
         (ufo-hit? u (rest lom)))]))
...
; UFO Missile -> Boolean
; Checks if the missile hit the UFO near or at its center of gravity.
(define (missile-hit-ufo? m u)
  (<= (distance-posns m u) UFO-RADIUS))

(check-expect (missile-hit-ufo? missile0 ufo0) #false)
(check-expect (missile-hit-ufo? missile1 ufo2) #true)
(check-expect (missile-hit-ufo? missile2 ufo3) #true)
(check-expect (missile-hit-ufo? missile3 ufo1) #true)
(check-expect (missile-hit-ufo? missile4 ufo4) #true)
(check-expect (missile-hit-ufo? missile0 ufo4) #false)
(check-expect (missile-hit-ufo? missile1 ufo3) #false)
(check-expect (missile-hit-ufo? missile2 ufo2) #false)
(check-expect (missile-hit-ufo? missile3 ufo0) #false)
(check-expect (missile-hit-ufo? missile4 ufo1) #false)
...
(check-expect (distance-posns (make-posn 0 0) (make-posn 3 4)) 5)
(check-expect (distance-posns (make-posn 4 3) (make-posn 0 0)) 5)
(check-expect (distance-posns (make-posn -3 -3) (make-posn 3 5)) 10)
(check-within (distance-posns (make-posn -1 3) (make-posn 2 -4)) (sqrt 58)
              0.0001)
