...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname ex278) (read-case-sensitive #t) (teachpacks ((lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp")) #f)))
; Graphical constants
(define RADIUS 5)
...
(define (tock gs)
  (local ((define future-worm (worm-tock (gs-worm gs))))
    (if (equal? (first (worm-segments future-worm)) (gs-food gs))
        (make-gs (extend-worm (gs-worm gs)) (food-create (gs-food gs)))
        (make-gs future-worm (gs-food gs)))))
...
(define (worm-render w im)
  (foldr place-segment im (worm-segments w)))

(check-expect (worm-render w0 BACKG)
              (place-segment (make-posn COORD-CENTER COORD-CENTER) BACKG))
(check-expect (worm-render w1 BACKG)
              (place-segment (make-posn 15 10)
                (place-segment (make-posn 15 11)
                  (place-segment (make-posn 15 12)
                    (place-segment (make-posn 16 12) BACKG)))))
...
; Number Number [NEList-of Posn] -> [NEList-of Posn]
...
(define (hit-wall? w)
  (local ((define head (first (worm-segments w))))
    (or (< (posn-x head) 0) (< (posn-y head) 0)
        (> (posn-x head) COORD-WIDTH) (> (posn-y head) COORD-WIDTH))))
